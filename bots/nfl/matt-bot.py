...
# TODO confirm these from CSV. Gotten from FanstasyPros
MAX_POS_RANK_WR = 200
MAX_POS_RANK_RB = 156
MAX_POS_RANK_TE = 97
MAX_POS_RANK_QB = 67
MAX_POS_RANK_K = 34
MAX_POS_RANK_DST = 31   # isn't this 32?
...
def experiment(undrafted_players: List[Player], drafted_players: List[Player]):
  ...
  # best_player = min(players_ranked, key=lambda p: p.rank * p.position_rank);
...
def score_player(player: Player, undrafted_players: List[Player], drafted_players: List[Player]):
  ...
  score = get_rank_percentile(player) * 0.35 + get_position_rank_percentile(player) * 0.35 + get_position_need(player) * 0.29 + get_position_demand(player, drafted_players) * 0.01

  # print(str(player.full_name) + " | need: " + str(position_need))
